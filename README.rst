...
API
==================
API 指的是 Application Programming Interface, 
中文稱作 應用程式介面, 
可以把它想像成一個服務生, 
今天你向服務生點餐, 他幫你把餐點需求送到廚房, 
再從廚房把你要的菜餚拿給你, 
推回程式上, 你把你的需求發給api, 他過去系統提取你需要的資料,
再從系統返回你需要的資料給你
...
::
    ...
    https://ithelp.ithome.com.tw/articles/10207897?sc=iThelpR
...
剛看到這個詞直覺反應, reset 是重來、重置, 其實在這裡把它看成 become 會更好, 

使用 git 無非就是想作為版本控制, 而 reset 更是箇中精華, 你在刪錯檔案或想回到某個版本的狀態就是靠它了, 

::

    git reset HEAD^

上面這個指令大家都知道, 是針對 HEAD 的位置退一個版本, 而要退幾個版本就在後面加幾個 ^ , 當然我們也可以直接把 ^ 換成版本號碼, 

::

    git reset <版本號碼>

這裡其實 git defult 已經偷偷幫我加了一個參數, 完整應該長這樣 

::

    git reset <版本號碼> --mixed

後面有三個參數
 - --mixed  不指定時的預設行為, 保留檔案變更
 - --soft  working tree 未改變, 僅移除commit
 - --hard  working tree 移除修改的內容, 完全回到上一版

這時候就會有問題, 如果今天我已經退版本, 發現退錯了, 

可是 commit 跟版號都已經被清除了, 沒有版號就回去退錯版本之前, 
...
這裡會顯示過去所有的版號和做過的事,  ``git log`` 只會顯示當前有的 commit 並不顯示已刪除或退版本的版號

::

    git reset ORIG_HEAD --hard

這個指令是一個秘密的紀錄點, 它會自動幫你記錄最近一次的危險操作, 讓你可以返回, 但就只有最近的那次, 

何謂危險操作呢？ 像是 rebase、merge ...等    這種會讓檔案有大變動的操作。
